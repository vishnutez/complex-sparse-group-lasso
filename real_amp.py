...
class RealAMP:
    """Real-valued Approximate Message Passing for sparse group lasso recovery.

    Accepts complex-valued inputs and converts them to real via the standard
    isomorphisms:
      - vectors: [x1, x2, ...] -> [x1r, x1i, x2r, x2i, ...]
      - matrices: A -> [[Ar, -Ai], [Ai, Ar]] (block real form)

    This lets the real AMP operate on the equivalent real-valued system while
    still accepting complex sensing matrices and signals.
    """
    ...
    def __init__(
        ...
        # Convert complex inputs to real
        self.measurements = self._complex_vec_to_real_vec(measurements)
        self.sensing_matrix = self._complex_mat_to_real_mat(sensing_matrix)
        self.signal_true = self._complex_vec_to_real_vec(signal_true)
        self.prior_sample = self._complex_vec_to_real_vec(prior_sample)
        self.true_support = (np.abs(self.signal_true) > 0) * 1.0

        # Dimensions
        self.batch_size = self.measurements.shape[0]
        self.signal_dim = self.sensing_matrix.shape[-1]
        self.num_measurements = self.measurements.shape[-2]
        self.measurement_ratio = self.num_measurements / self.signal_dim

        # Fixed solver parameters
        self.noise_variance = noise_variance / 2  # real noise variance is half the complex
        self.step_size = step_size
        self.tau_estimation = tau_estimation

        self.algo = None

        # Group structure: each complex group of size g becomes a real group of size 2g
        self.group_size = 2 * group_size
        self.num_groups = self.signal_dim // self.group_size
    ...
    # ------------------------------------------------------------------
    # Complex-to-real conversion helpers
    # ------------------------------------------------------------------

    def _complex_vec_to_real_vec(self, complex_vec):
        """Interleave real and imaginary parts: [x1, x2] -> [x1r, x1i, x2r, x2i]."""
        return np.stack(
            [np.real(complex_vec), np.imag(complex_vec)], axis=-2
        ).reshape(*complex_vec.shape[:-2], -1, 1)

    def _complex_mat_to_real_mat(self, complex_mat):
        """Block real form: A -> [[Ar, -Ai], [Ai, Ar]], giving (b, n, N) -> (b, 2n, 2N)."""
        A_r = np.real(complex_mat)
        A_i = np.imag(complex_mat)
        even_rows = np.stack([ A_r, -A_i], axis=-1).reshape(*complex_mat.shape[:-1], -1)
        odd_rows  = np.stack([ A_i,  A_r], axis=-1).reshape(*complex_mat.shape[:-1], -1)
        return np.stack([even_rows, odd_rows], axis=-2).reshape(
            *complex_mat.shape[:-2], -1, even_rows.shape[-1]
        )
    ...
    def _soft_threshold(self, x, threshold=0.0):
        """Element-wise real soft thresholding: sign(x) * max(|x| - threshold, 0)."""
        return np.sign(x) * np.maximum(np.abs(x) - threshold, 0.0)

    def _transpose(self, matrix):
        """Transpose of a batched matrix: (batch, m, n) -> (batch, n, m)."""
        return matrix.swapaxes(-1, -2)
    ...
    def _denoiser_avg_divergence(self, estimate, element_threshold=0.0, group_threshold=0.0):
        ...
        if np.any(group_threshold > 0.0):
            ...
            clipped_group_norms = np.maximum(group_norms, group_threshold)
            group_divergence = group_activity_mask * element_activity_mask * (
                1 - group_threshold / clipped_group_norms * (
                    1 - lasso_denoised_groups ** 2 / clipped_group_norms ** 2
                )
            )
        else:
            group_divergence = element_activity_mask

        group_divergence = group_divergence.sum(axis=(-2, -1), keepdims=True)  # (batch, 1, 1)
    ...
    def _amp_step(self, element_threshold, group_threshold):
        ...
        self.denoiser_input = (
            ...
            + self._transpose(self.sensing_matrix) @ self.residual
        )
    ...
    def _ista_step(self, element_threshold, group_threshold):
        ...
        self.denoiser_input = (
            self.signal_estimate
            + self.step_size * self._transpose(self.sensing_matrix) @ self.residual
        )
        self.signal_estimate = self.denoise(
            self.denoiser_input, element_threshold, group_threshold
        )
    ...
    def _se_step(self, element_threshold, group_threshold):
        """Compute one SE update and append results."""
        tau = np.sqrt(self.se_effective_noise[-1])

        denoiser_input = self.prior_sample + tau * np.random.randn(*self.prior_sample.shape)
    ...
    def run(
        ...
        Args:
            ...
            warm_start: If True, continue from current state.
            tau_scale: Multiplicative scale applied to the empirical tau estimate.
            tolerance: Stop early when SE MSE change falls below this value.
            step_size: Overrides self.step_size (ignored for ISTA).
            group_threshold_freq: Period for group thresholding in amp_sparse_group_lasso.
        """
